...
| `SettingsSectionView` | Разрешения, тема, поведение каталога, логирование и VPN-автоматизация. | Сохранение настроек с debounce. |
...
Android разделяет личный и рабочий профили, поэтому операции в рабочем профиле выполняются через профильную Activity. В Agnosia эту роль выполняет `DummyActivity`: она прозрачная, исключена из списка недавних задач и принимает набор внутренних `AgnosiaActions`.
...
| Временное управление VPN | Личный | Позволяет Agnosia кратко занять VPN-слот и отключить активный VPN. |
...
| Поверх окон | Личный | Нужен overlay-индикатору для VPN-сценария. |
...
Запросы разрешений используют разные Android-механизмы. Уведомления запрашиваются через runtime permission `POST_NOTIFICATIONS`, VPN - через `VpnService.prepare()`, Usage Stats и установка APK открываются из рабочего профиля, а overlay-доступ открывается через страницу настроек приложения, где пользователь вручную включает `SYSTEM_ALERT_WINDOW`. После возврата в приложение view model перечитывает разрешения на `OnResume`, если этот тип доступа меняется вне Agnosia.
...
Сервис не принимает ввод, не забирает фокус и удаляет окно при завершении сценария. В текущей реализации обработчик `WorkAppFrozen` всегда пытается скрыть overlay через bind к сервису, но публичная обёртка для запуска action `SHOW_VPN_OVERLAY` не используется остальными компонентами. Поэтому overlay-доступ описан как вспомогательная возможность: отсутствие видимого квадрата не блокирует отключение VPN, запуск рабочего приложения, заморозку или восстановление VPN.
...
| `OverlayVpnService` | Реализует показ и скрытие overlay-индикатора VPN-сценария; сейчас основной поток вызывает только скрытие после `WorkAppFrozen`. |
