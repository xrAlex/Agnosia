...
Список обновляется при раскрытии раздела, возврате в Agnosia, вручную в рабочем профиле и после попытки изменения, включая неуспешную. Во время операции команды заблокированы; при ошибке перечитывания старые строки удаляются. В рабочем профиле поиск работает по локализованной подписи и полному имени разрешения. В личном профиле каждая строка показывает только название, техническое имя и описание; поиск, счётчик и ручное обновление скрыты.
...
Третий путь онбординга: `StartDirectProvisioningCommand` → `IOnboardingPlatformService.StartDirectProvisioningAsync` → `AndroidProvisioningCoordinator` → `DirectProfileProvisioningWorkflow`. Все способы создания используют общий запрет одновременного запуска. Root-путь проверяет `su` через `id -u` и не зависит от `IsProvisioningAllowed` для системного мастера.

`AndroidRootCommandRunner` выполняет команды вне UI-потока с отдельным ограниченным чтением stdout/stderr. Внутри root-сеанса `timeout -s KILL 75` ограничивает время команды, внешний таймаут ожидания составляет 90 секунд. До подтверждения запуска этого сеанса в stdin передаётся только безопасный маркер; задержанное разрешение `su` после отмены не может запустить изменение системы. После отправки команды runner ждёт завершения привилегированного процесса и обоих потоков; отмена останавливает следующие команды. Секрет передаётся через stdin, отсутствует в аргументе `su -c` и не включается в логи или ошибки UI.

Последовательность: `dumpsys user` / `dpm list-owners`, `pm create-user --profileOf … --managed Agnosia`, `pm install-existing --user …`, проверка `pm path`, `dpm set-profile-owner`, повторная проверка владельца, `am start-user -w` и адресный broadcast завершения настройки. Команды `pm`/`dpm`/`am` выполняются через `exec /system/bin/cmd package|device_policy|activity`: это сохраняет PID фактического клиента Android для [реализации timeout в Android 12](https://android.googlesource.com/platform/external/toybox/+/refs/heads/android12-release/toys/other/timeout.c), которая завершает непосредственного потомка. Использование `install-existing` сохраняет установленный APK, включая split APK, и избегает повторной установки личной копии. Глобальные ограничения числа пользователей и проверка APK не меняются.

`DirectProfileProvisioningCommands` проверяет формат shell-ответов. Parent ID берётся из пользователя процесса приложения. ID созданного профиля сверяется с managed-флагом, parent ID и serial пользователя из `dumpsys user`. В `direct_profile_provisioning` синхронно сохраняются версия формата, parent ID/serial, profile ID/serial и этап; ключ сохраняется той же транзакцией preferences. При повторном запуске продолжается только собственная настройка с совпадающей идентичностью и ожидаемым владельцем. Профиль с повторно использованным числовым ID, неизвестным serial или другим владельцем не принимается. После частичного сбоя профиль сохраняется для повторной попытки либо ручного удаления; автоматического удаления пользователей нет.

`DirectProfileSetupReceiver` не экспортируется. Root адресует его явно в созданном пользователе, с включением остановленного пакета. Receiver проверяет managed-профиль, владельца, целевой user ID и формат ключа; существующий ключ допускает только повтор того же значения. Затем синхронно сохраняет ключ, вызывает `AndroidStartup.EnforceWorkProfilePolicies(context, true)` и возвращает подтверждение. Успех требует подтверждения receiver и свежего подписанного `ProfilePing` с проверкой владельца. Пока журнал не завершён, dashboard сохраняет ключ и состояние восстановления и не объявляет профиль готовым, даже если частично настроенная копия уже отвечает.

Перед каждой командой, изменяющей состояние системы, журнал получает `AwaitingReboot` и текущий `Settings.Global.BootCount`. Только подтверждённый результат снимает эту отметку. При таймауте, разрыве root-сеанса или смерти приложения повторная попытка заблокирована до следующей загрузки устройства: Binder-сервис Android мог принять команду до завершения shell-клиента. Обычное и офлайн-создание проверяют ту же отметку перед очисткой журнала и сменой ключа. После перезагрузки снова проверяются serial, parent и владелец; профиль без достоверно сохранённой идентичности требует ручного восстановления. Журнал `Complete` при ещё существующем профиле запрещает повторную настройку готового профиля.

Форматы команд проверены по [AOSP PackageManagerShellCommand](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/android12-release/services/core/java/com/android/server/pm/PackageManagerShellCommand.java), [DevicePolicyManagerServiceShellCommand](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/android12-release/services/devicepolicy/java/com/android/server/devicepolicy/DevicePolicyManagerServiceShellCommand.java) и [ActivityManager](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/android12-release/core/java/android/app/ActivityManager.java). На прошивке с неизвестным форматом ответов операция останавливается с ошибкой этапа. Проверка на устройстве с root необходима для подтверждения совместимости конкретного `su` и прошивки.
...
Одновременные проверки разрешений из dashboard и онбординга используют один выполняющийся запрос: повторный `QueryPermissions` с приоритетом `Refresh` отменил бы предыдущий в Android scheduler. На время обработки «Открыть приложения» включается busy-состояние, подавляющее автоматическое обновление при возврате из командной Activity. После сохранения завершения dashboard обновляется без повторной проверки уже прочитанных разрешений. Ошибки отображаются в самом онбординге, а недостающие разрешения возвращают пользователя к шагу их выдачи.
